...
def check_last_day(df_hl, df):
    ...
    rtn_cols = ['code', 'name', 'current_date','current_close','delta_to_long_enter',
                'long_enter','long_quit','long_expect_ear_perct','up_p','down_p','up_cnt','down_cnt','day_to_up',
                'day_to_down','up_date','down_date','perc_to_up','perc_to_down','last_is_h_or_l',
                'short_enter','short_quit','short_expect_ear_perct','delta_to_short_enter']
    ...
    for i in range(df_hl.__len__()-1):
        ...
        if df_hl.iloc[i].close_round < last_close and last_close < df_hl.iloc[i+1].close_round:
            ...
            down_date = datetime.datetime.strptime(down_date, '%Y-%m-%d').date()
...
def main():
    ...
    parser.add_option("-b", "--calc_base", action="store_true", dest="calc_base", default=False,
                      help="Step1. generate base High Low point for each stock, time consuming.")

    parser.add_option("-t", "--calc_today", action="store_true", dest="calc_today", default=False,
                      help="Step2. find current price vs HL.")


    parser.add_option("-s", "--today_selection", action="store_true", dest="today_selection", default=False,
    ...
    if stock_global == 'AG':
        ...
        df_code_name_map = finlib.Finlib().add_market_to_code(df=df_code_name_map)
        valid_df = finlib.Finlib().remove_garbage(df_code_name_map, code_filed_name='code', code_format='C2D6')
    else:
        # '''
    ...
    if calc_base or calc_today:
        ...
        for code in valid_df['code']:
            ...
            if not re.match(".*DAY_Global/AG", root_dir):
                ...
                df = pd.read_csv(inputF, skiprows=1, converters={'code': str}, header=None, names=['code', 'date', 'o', 'h', 'l', 'close', 'vol', 'amnt', 'tnv'])


            #adding name column
            df = pd.merge(df, valid_df, on='code', how='inner', suffixes=('', '_x'))
            ...
            if calc_base:
                ...
                if (not force_run_global) and finlib.Finlib().is_cached(tmp_base_f, 6):
