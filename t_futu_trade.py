...
def buy_limit(quote_ctx, trd_ctx,df_stock_info,
              code, drop_threshold=0.19, pwd_unlock='123456',
              trd_env = ft.TrdEnv.SIMULATE, time_sleep=4):
    ...
    ret, order_table = trd_ctx.place_order(price=price_to_order, qty=lot_size, code=code,
                              trd_side=ft.TrdSide.BUY,
                              trd_env=trd_env,  order_type=order_type)
...
if __name__ == '__main__':
    ...
    for index, row in df_input.iterrows():
        ...
        #place order
        buy_limit(quote_ctx=quote_ctx,trd_ctx=trd_ctx,df_stock_info=df_stock_info,
                  code=code,drop_threshold=drop_threshold,pwd_unlock=pwd_unlock, trd_env=trd_env, time_sleep= 4)
