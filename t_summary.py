...
def refine_df(input_df,
              has_db_record=False,
              force_pass=False,
              force_pass_open_reason='',
              insert_buy_record_to_db=False,
              debug=False):
    ...
    logging.info("function refine_df, input df records " +
                 str(input_df.__len__()) + " total stocks in the list: " +
                 str(input_df.__len__()))

    #df_rtn = pd.DataFrame(columns=input_df.columns).rename(columns={'op': 'hit_ptn_cnt'}).drop('op_rsn', axis=1)
    df_rtn = pd.DataFrame(columns=input_df.columns).rename(
        columns={'op': 'hit_ptn_cnt'})
    ...
    for i in input_df['code'].unique():
        ...
        if c.__len__() > 0:
            ...
            if has_db_record and insert_buy_record_to_db:  # AG only
                ...
                # pass #
                    ...
                    continue
                elif (avg2 > 0.02) and (avg5 > 0.02) and (
                        sum2uc > sum2dc * 3) and (sum5uc > sum5dc * 3):
        ...
        if force_pass:
            ...
            c_op_rsn = 'refine_df_func. ' + ','.join(
                c['op_rsn'].values.tolist())

        c_date = str(c['date'].values[0])
        c_status = 'OPEN'

        if pd.isnull(c_close_p) or float(c_close_p) == 0.0:
            logging.info("not processing as c_close_p is 0. " + c_code + " " +
                         c_name + " " + c_date)
            continue
        ...
        if insert_buy_record_to_db:
            ...
            if open_stocks.__len__() > 0:
                logging.info("Have OPEN order on code " + c_code +
                             ", +1 its buy_cnt")
                ...
                data_sql['buy_reason'] = bs[0:
                                            3199]  #var char 3200, length limit
                ...
                logging.info('buy candidate,' + c_code + ' ' + c_name + ' ' +
                             c_close_p + ' ' + c_date)
        ...
        try:
            ...
            logging.info("re-calc op_strength_group_sum to " +
                         str(op_strength_group_sum) + ", " + c_code + " " +
                         c_name)
    ...
    df_rtn = df_rtn.sort_values(by=['hit_ptn_cnt', 'op_strength'],
                                ascending=[False,
...
def combin_filter(df, post_combine=False, debug=False):
    ...
    for col in df.columns:
        if post_combine:
            if not re.match('.*_y[0-9]+',
                            col):  #remove duplicate name *_y0 from combined df
                cols.append(col)
        else:
...
def generate_result_csv(full_combination=False, debug=False):
    ...
    if (os.path.isfile(f_pv_no_filter)
        ) and os.stat(f_pv_no_filter).st_size >= 10:  # > 10 bytes
        df_pv_no_filter = pd.read_csv(f_pv_no_filter,
                                      dtype=str,
                                      encoding="utf-8")
        df_pv_no_filter.drop_duplicates(inplace=True)
        df_pv_no_filter = finlib.Finlib().remove_garbage(
            df_pv_no_filter, code_filed_name='code', code_format='C2D6')
    ...
    if 'df_pv_no_filter' in locals():
        ...
        df_pv_no_filter = df_pv_no_filter.loc[
            df_pv_no_filter['close_p'] != '0.0']
        df_pv_no_filter = refine_df(df_pv_no_filter,
                                    has_db_record=False,
                                    force_pass=True,
                                    insert_buy_record_to_db=False,
                                    debug=debug)
        ...
        df_pv_break = df_pv_no_filter[df_pv_no_filter['op_rsn'].str.contains(
            '_pvbreak_')]

        df_pv_break = refine_df(df_pv_break,
                                has_db_record=False,
                                force_pass=True,
                                insert_buy_record_to_db=False,
                                debug=debug)
        ...
        df_max_daily_increase = df_pv_no_filter[
            df_pv_no_filter['op_rsn'].str.contains('_max_daily_increase')]
        df_max_daily_increase = df_max_daily_increase.reset_index()
        df_max_daily_increase = refine_df(df_max_daily_increase,
                                          has_db_record=False,
                                          force_pass=True,
                                          insert_buy_record_to_db=False,
                                          debug=debug)
        df_max_daily_increase = finlib.Finlib().remove_garbage(
            df_max_daily_increase, code_filed_name='code', code_format='C2D6')
        logging.info("\t df_max_daily_increase length " +
                     str(df_max_daily_increase.__len__()))

        #====== max_daily_decrease
        logging.info("loading df_max_daily_decrease")
        df_max_daily_decrease = df_pv_no_filter[
            df_pv_no_filter['op_rsn'].str.contains('_max_daily_decrease')]
        df_max_daily_decrease = df_max_daily_decrease.reset_index()
        df_max_daily_decrease = refine_df(df_max_daily_decrease,
                                          has_db_record=False,
                                          force_pass=True,
                                          insert_buy_record_to_db=False,
                                          debug=debug)
        df_max_daily_decrease = finlib.Finlib().remove_garbage(
            df_max_daily_decrease, code_filed_name='code', code_format='C2D6')
        logging.info("\t df_max_daily_decrease length " +
                     str(df_max_daily_decrease.__len__()))

        #====== decrease gap
        logging.info("loading df_decrease_gap")
        df_decrease_gap = df_pv_no_filter[
            df_pv_no_filter['op_rsn'].str.contains('_decrease_gap')]
        df_decrease_gap = df_decrease_gap.reset_index()
        df_decrease_gap = refine_df(df_decrease_gap,
                                    has_db_record=False,
                                    force_pass=True,
                                    insert_buy_record_to_db=False,
                                    debug=debug)
        df_decrease_gap = finlib.Finlib().remove_garbage(
            df_decrease_gap, code_filed_name='code', code_format='C2D6')
        logging.info("\t df_decrease_gap length " +
                     str(df_decrease_gap.__len__()))

        #====== increase gap
        logging.info("loading df_increase_gap")
        df_increase_gap = df_pv_no_filter[
            df_pv_no_filter['op_rsn'].str.contains('_increase_gap')]
        df_increase_gap = df_increase_gap.reset_index()
        df_increase_gap = refine_df(df_increase_gap,
                                    has_db_record=False,
                                    force_pass=True,
                                    insert_buy_record_to_db=False,
                                    debug=debug)
        df_increase_gap = finlib.Finlib().remove_garbage(
            df_increase_gap, code_filed_name='code', code_format='C2D6')
        logging.info("\t df_increase_gap length " +
                     str(df_increase_gap.__len__()))

        #====== 52 week low price
        logging.info("loading df_low_price_year")
        df_low_price_year = df_pv_no_filter[
            df_pv_no_filter['op_rsn'].str.contains('_pvbreak_lp_year')]
        ...
        df_low_price_year = refine_df(df_low_price_year,
                                      has_db_record=False,
                                      force_pass=True,
                                      insert_buy_record_to_db=False,
                                      debug=debug)
        df_low_price_year = finlib.Finlib().remove_garbage(
            df_low_price_year, code_filed_name='code', code_format='C2D6')
        logging.info("\t df_low_price_year length " +
                     str(df_low_price_year.__len__()))

        #====== 52 week low volume
        logging.info("loading df_low_vol_year")
        df_low_vol_year = df_pv_no_filter[
            df_pv_no_filter['op_rsn'].str.contains('_pvbreak_lv_year')]
        ...
        df_low_vol_year = refine_df(df_low_vol_year,
                                    has_db_record=False,
                                    force_pass=True,
                                    insert_buy_record_to_db=False,
                                    debug=debug)
        df_low_vol_year = finlib.Finlib().remove_garbage(
            df_low_vol_year, code_filed_name='code', code_format='C2D6')
        logging.info("\t df_low_vol_year length " +
                     str(df_low_vol_year.__len__()))
    ...
    if stock_global in ['AG']:
        df_fenghong = pd.read_csv(f_fenghong,
                                  converters={'code': str},
                                  encoding="utf-8")
        df_fenghong = df_fenghong[df_fenghong['score'] >= 0.8]
        df_fenghong = finlib.Finlib().remove_garbage(df_fenghong,
        ...
        df_peg_ps = pd.read_csv(f_fund_peg_ps,
                                converters={'code': str},
                                encoding="utf-8")
        df_peg_ps = finlib.Finlib().add_market_to_code(df=df_peg_ps)
        df_peg_ps = finlib.Finlib().remove_garbage(df_peg_ps,
                                                   code_filed_name='code',
                                                   code_format='C2D6')
        logging.info("\t df_peg_ps length " + str(df_peg_ps.__len__()))

        df_support_resist_line_today = pd.read_csv(f_support_resist_line_today,
                                                   converters={'code': str},
                                                   encoding="utf-8")
        df_support_resist_line_today = finlib.Finlib().remove_garbage(
            df_support_resist_line_today,
            code_filed_name='code',
            code_format='C2D6')
        logging.info("\t df_support_resist_line_today length " +
                     str(df_support_resist_line_today.__len__()))
        ...
        df_disclosure_date_notify = pd.read_csv(f_disclosure_date_notify,
                                                ...
                                                encoding="utf-8")
        df_disclosure_date_notify = finlib.Finlib().remove_garbage(
            df_disclosure_date_notify,
            code_filed_name='code',
            code_format='C2D6')
        logging.info("\t df_disclosure_date_notify length " +
                     str(df_disclosure_date_notify.__len__()))

        df_macd_m = pd.read_csv(f_macd_m, encoding="utf-8")
        df_macd_m = df_macd_m[df_macd_m.action.str.contains('BUY')]
        df_macd_m = finlib.Finlib().remove_garbage(df_macd_m,
                                                   code_filed_name='code',
                                                   code_format='C2D6')
        ...
        df_macd_w = finlib.Finlib().remove_garbage(df_macd_w,
                                                   code_filed_name='code',
                                                   code_format='C2D6')
        ...
        df_macd_d = finlib.Finlib().remove_garbage(df_macd_d,
                                                   code_filed_name='code',
                                                   code_format='C2D6')
        ...
        df_kdj_m = finlib.Finlib().remove_garbage(df_kdj_m,
                                                  code_filed_name='code',
                                                  code_format='C2D6')
        ...
        df_kdj_w = finlib.Finlib().remove_garbage(df_kdj_w,
                                                  code_filed_name='code',
                                                  code_format='C2D6')
        ...
        logging.info("\t df_concept_top length " +
                     str(df_concept_top.__len__()))
        ...
        if (os.path.isfile(f_fund) and os.stat(f_fund).st_size >= 10):
            ...
            df_fund = df_fund[
                df_fund['result_value_quarter_fundation'] > str(0.8)]
            df_fund = finlib.Finlib().remove_garbage(df_fund,
                                                     code_filed_name='code',
                                                     code_format='C2D6')
            ...
            cols = [
                'code', 'name', 'result_value_sum', 'result_value_today',
                'result_value_quarter_fundation', 'pe', 'amount', 'esp',
                'profit'
            ]
        ...
        if (os.path.isfile(f_fund_2)
            ...
            df_fund_2 = finlib.Finlib().remove_garbage(df_fund_2,
                                                       code_filed_name='code',
                                                       code_format='C2D6')
        ...
        if (os.path.isfile(f_pe_pb_roe_history)
            ) and os.stat(f_pe_pb_roe_history).st_size >= 10:  # > 10 bytes
            df_pe_pb_roe_history = pd.read_csv(f_pe_pb_roe_history,
                                               encoding="utf-8")
            df_pe_pb_roe_history = finlib.Finlib().remove_garbage(
                df_pe_pb_roe_history,
                code_filed_name='code',
                code_format='C2D6')
            logging.info("\t df_pe_pb_roe_history length " +
                         str(df_pe_pb_roe_history.__len__()))

            df_pe_pb_roe_history = df_pe_pb_roe_history.drop_duplicates()
            df_pe_pb_roe_history = df_pe_pb_roe_history.reset_index().drop(
                'index', axis=1)
        else:
        ...
        if (os.path.isfile(f_whitehorse)
            ) and os.stat(f_whitehorse).st_size >= 10:  # > 10 bytes
            df_whitehorse = pd.read_csv(f_whitehorse, encoding="utf-8")
            df_whitehorse = finlib.Finlib().remove_garbage(
                df_whitehorse, code_filed_name='code', code_format='C2D6')
            logging.info("\t df_whitehorse length " +
                         str(df_whitehorse.__len__()))
        ...
        logging.info("loading f_freecashflow_price_ratio, " +
                     f_freecashflow_price_ratio)
        if (os.path.isfile(f_freecashflow_price_ratio)) and os.stat(
                f_freecashflow_price_ratio).st_size >= 10:  # > 10 bytes
            df_freecashflow_price_ratio = pd.read_csv(
                f_freecashflow_price_ratio, encoding="utf-8")
            df_freecashflow_price_ratio = finlib.Finlib().remove_garbage(
                df_freecashflow_price_ratio,
                code_filed_name='code',
                code_format='C2D6')
            logging.info("\t df_freecashflow_price_ratio length " +
                         str(df_freecashflow_price_ratio.__len__()))

            df_freecashflow_price_ratio = df_freecashflow_price_ratio.drop_duplicates(
            )
            df_freecashflow_price_ratio = df_freecashflow_price_ratio.reset_index(
            ).drop('index', axis=1)
        else:
        ...
        if (os.path.isfile(f_whitehorse)
            ...
            df_hen_cow = finlib.Finlib().remove_garbage(df_hen_cow,
                                                        code_filed_name='code',
                                                        code_format='C2D6')
        ...
        if (os.path.isfile(f_pv_db_buy_filter)
            ) and os.stat(f_pv_db_buy_filter).st_size >= 10:  # > 10 bytes
            df_pv_db_buy_filter = pd.read_csv(f_pv_db_buy_filter,
                                              dtype=str,
                                              encoding="utf-8")
            df_pv_db_buy_filter.drop_duplicates(inplace=True)

            df_pv_db_buy_filter.sort_values('2mea',
                                            ascending=False,
                                            inplace=True)
            # already sorted by Increase_2D in t_daily_pattern_Hit_Price_Volume.py
            df_pv_db_buy_filter = df_pv_db_buy_filter.loc[
                df_pv_db_buy_filter['close_p'] != '0.0']
            len_df_pv_db_buy_filter_0 = str(df_pv_db_buy_filter.__len__())
            df_pv_db_buy_filter = finlib.Finlib().remove_garbage(
                df_pv_db_buy_filter,
                code_filed_name='code',
                code_format='C2D6')
            logging.info("\t df_pv_db_buy_filter length " +
                         str(df_pv_db_buy_filter.__len__()))
        else:
            logging.info("ERROR: NOT found file " + f_pv_db_buy_filter)
            #exit(0)

        logging.info("loading  df_pv_db_sell_filter")
        if (os.path.isfile(f_pv_db_sell_filter)
            ) and os.stat(f_pv_db_sell_filter).st_size >= 10:  # > 10 bytes
            df_pv_db_sell_filter = pd.read_csv(f_pv_db_sell_filter,
                                               dtype=str,
                                               encoding="utf-8")
            df_pv_db_sell_filter.drop_duplicates(inplace=True)

            df_pv_db_sell_filter.sort_values('2mea',
                                             ascending=True,
                                             inplace=True)
            # already sorted by Increase_2D in t_daily_pattern_Hit_Price_Volume.py
            df_pv_db_sell_filter = df_pv_db_sell_filter.loc[
                df_pv_db_sell_filter['close_p'] != '0.0']
            len_df_pv_db_sell_filter_0 = str(df_pv_db_sell_filter.__len__())
            df_pv_db_sell_filter = finlib.Finlib().remove_garbage(
                df_pv_db_sell_filter,
                code_filed_name='code',
                code_format='C2D6')
            logging.info("\t df_pv_db_sell_filter length " +
                         str(df_pv_db_sell_filter.__len__()))
    ...
    if stock_global in ['MG', 'US', 'CN']:
        df_sp500_index = pd.read_csv(f_sp500_index,
                                     converters={'code': str},
                                     encoding="utf-8")
        arr.append('df_sp500_index')

        df_sp400_index = pd.read_csv(f_sp400_index,
                                     converters={'code': str},
                                     encoding="utf-8")
        arr.append('df_sp400_index')

        df_nasdqa100_index = pd.read_csv(f_nasdqa100_index,
                                         converters={'code': str},
                                         encoding="utf-8")
        arr.append('df_nasdqa100_index')

        df_dow_index = pd.read_csv(f_dow_index,
                                   converters={'code': str},
                                   encoding="utf-8")
    ...
    if stock_global in ['AG']:
        if 'df_pv_break' in locals(
        ):  # it has already been added to arr, do not arr.append again.
        ...
        if 'df_pv_db_buy_filter' in locals():  #code already in 'SH/SZ' format
            ...
            df_pv_db_buy_filter = finlib.Finlib().df_filter(
                df_pv_db_buy_filter)
        ...
        if 'df_pv_db_sell_filter' in locals():  #code already in 'SH/SZ' format
            ...
            df_pv_db_sell_filter = finlib.Finlib().df_filter(
                df_pv_db_sell_filter)
        ...
        if 'df_industry_top' in locals():
            ...
            df_industry_top = finlib.Finlib().add_market_to_code(
                df_industry_top)
        ...
        if 'df_peg_ps' in locals():
            ...
            df_peg_ps = finlib.Finlib().df_filter(
                df_peg_ps)  #market code should in code, e.g.  SH600001
            arr.append('df_peg_ps')

        if 'df_support_resist_line_today' in locals():
            logging.info("filtering df_support_resist_line_today")
            df_support_resist_line_today = finlib.Finlib().df_filter(
                df_support_resist_line_today)
        ...
        if 'df_disclosure_date_notify' in locals():
            ...
            df_disclosure_date_notify = finlib.Finlib().df_filter(
                df_disclosure_date_notify)
    ...
    if 'df_ann' in locals():
        if stock_global in ['AG']:
            ...
            df_ann = refine_df(
                ...
                has_db_record=False,
                force_pass=True,
                force_pass_open_reason='announcement',
                insert_buy_record_to_db=False,
                debug=debug
            )  #because the df_ann doesn't have duplicate on 'code' column
    ...
    if 'df_pv_no_filter' in locals():
        if stock_global in ['AG']:
            ...
            logging.info("Len df_pv_no_filter " + str(len_df_pv_no_filter_0) +
                         " => " + str(df_pv_no_filter.__len__()))
    ...
    if 'df_pv_db_buy_filter' in locals():
        if stock_global in ['AG']:
            ...
            df_pv_db_buy_filter = refine_df(df_pv_db_buy_filter,
                                            has_db_record=False,
                                            force_pass=True,
                                            insert_buy_record_to_db=False,
                                            debug=debug)
            logging.info("Len df_pv_db_buy_filter " +
                         str(len_df_pv_db_buy_filter_0) + " => " +
                         str(df_pv_db_buy_filter.__len__()))

        else:
            logging.info("will refine_df df_pv_db_buy_filter")
            df_pv_db_buy_filter = refine_df(df_pv_no_filter,
                                            has_db_record=False,
                                            force_pass=True,
                                            insert_buy_record_to_db=False,
                                            debug=debug)
    ...
    skip_sets = ('df_zz500', 'df_sme', 'df_hs300', 'df_gem',
                 'df_high_price_year'
                 )  #remove sz50 as 'df_sz50 df_sme' would make sense.
    buy_sets = ('df_fund', 'df_pv_break', 'df_low_price_year',
                'df_low_vol_year')
    ...
    df_dict["df_freecashflow_price_ratio"] = {
        "term": "LONG TERM",
        "price": "CHEAP"
    }
    ...
    df_dict["df_pv_db_sell_filter"] = {
        "term": "SHORT TERM",
        "price": "EXPENSIVE"
    }
    df_dict["df_pv_no_filter"] = {"term": "SHORT TERM", "price": "NA"}
    df_dict["df_support_resist_line_today"] = {
        "term": "SHORT TERM",
        "price": "CHEAP"
    }
    df_dict["df_disclosure_date_notify"] = {
        "term": "SHORT TERM",
        "price": "NA"
    }
    ...
    df_dict["df_macd_m"] = {
        "term": "LONG TERM",
        "price": "CHEAP"
    }  #_m =>Month, is long term for macd.
    ...
    if (stock_global in ['AG']) and (not full_combination):  #for AG
        ...
        if 'df_whitehorse' in locals():
            df_combined_year = pd.concat(
                [df_combined_year, df_whitehorse],
                sort=False).drop_duplicates().reset_index().drop('index',
                                                                 axis=1)
            df_reduced_year = pd.merge(df_reduced_year,
                                       df_whitehorse,
                                       on='code',
                                       how='inner',
                                       suffixes=('', '_x')).drop('name_x',
                                                                 axis=1)
            logging.info("After df_whitehorse, df_combined_year " +
                         str(df_combined_year.__len__()) +
                         ", df_reduced_year " + str(df_reduced_year.__len__()))
            #arr.remove('df_whitehorse')

        if 'df_hen_cow' in locals():
            df_combined_year = pd.concat(
                [df_combined_year, df_hen_cow],
                sort=False).drop_duplicates().reset_index().drop('index',
                                                                 axis=1)
            df_reduced_year = pd.merge(df_reduced_year,
                                       df_hen_cow,
                                       ...
                                       suffixes=('', '_x')).drop('name_x',
                                                                 axis=1)
            logging.info("After df_hen_cow, df_combined_year " +
                         str(df_combined_year.__len__()) +
                         ", df_reduced_year " + str(df_reduced_year.__len__()))
        ...
        if 'df_fenghong' in locals():
            ...
            df_reduced_year = pd.merge(df_reduced_year,
                                       ...
                                       suffixes=('', '_x')).drop('name_x',
                                                                 axis=1)
            logging.info("After df_fenghong, df_combined_year " +
                         str(df_combined_year.__len__()) +
                         ", df_reduced_year " + str(df_reduced_year.__len__()))
        ...
        if 'df_peg_ps' in locals():
            df_combined_quarter = pd.concat(
                [df_combined_quarter, df_peg_ps],
                sort=False).drop_duplicates().reset_index().drop('index',
                                                                 axis=1)
            df_reduced_quarter = pd.merge(df_reduced_quarter,
                                          df_peg_ps,
                                          on='code',
                                          how='inner',
                                          suffixes=('', '_x')).drop('name_x',
                                                                    axis=1)
            logging.info("After df_peg_ps, df_combined_quarter " +
                         str(df_combined_quarter.__len__()) +
                         ", df_reduced_quarter " +
                         str(df_reduced_quarter.__len__()))
            #arr.remove('df_peg_ps')

        if 'df_freecashflow_price_ratio' in locals():
            df_combined_quarter = pd.concat(
                [df_combined_quarter, df_freecashflow_price_ratio],
                sort=False).drop_duplicates().reset_index().drop('index',
                                                                 axis=1)
            df_reduced_quarter = pd.merge(df_reduced_quarter,
                                          df_freecashflow_price_ratio,
                                          ...
                                          suffixes=('', '_x')).drop('name_x',
                                                                    axis=1)
            logging.info(
                "After df_freecashflow_price_ratio, df_combined_quarter " +
                str(df_combined_quarter.__len__()) + ", df_reduced_quarter " +
                str(df_reduced_quarter.__len__()))
            #arr.remove('df_freecashflow_price_ratio')

        if 'df_industry_top' in locals():
            df_combined_quarter = pd.concat(
                [df_combined_quarter, df_industry_top],
                sort=False).drop_duplicates().reset_index().drop('index',
                                                                 axis=1)
            df_reduced_quarter = pd.merge(df_reduced_quarter,
                                          df_industry_top,
                                          ...
                                          suffixes=('', '_x')).drop('name_x',
                                                                    axis=1)
            logging.info("After df_industry_top, df_combined_quarter " +
                         str(df_combined_quarter.__len__()) +
                         ", df_reduced_quarter " +
                         str(df_reduced_quarter.__len__()))
        ...
        df_reduced_quarter_year = pd.merge(df_combined_quarter,
                                           df_combined_year,
                                           on='code',
                                           how='inner',
                                           suffixes=('', '_x')).drop('name_x',
        ...
        df_reduced_quarter_year = df_reduced_quarter_year.drop_duplicates(
        ).reset_index().drop('index', axis=1)

        #
        #''' No need to filter every df with df_reduced_quarter_year which are the quarter and yearly good stock
        for a in arr:
            logging.info("=== Inner Merge  " + a +
                         " with df_reduced_quarter_year ====")
...
def ana_result():
    ...
    for line in fh:
        ...
        pass
    fh.close()
    df_rpt_short = pd.DataFrame.from_dict(dict).T.reset_index().rename(
        columns={"index": "code"})
    df_rpt_short = df_rpt_short.sort_values('cnt',
                                            ascending=False,
                                            inplace=False)
    df_rpt_short.to_csv(output_f, encoding='UTF-8', index=False)
    logging.info(__file__ + ": " + "saved " + output_f + " . len " +
                 str(df_rpt_short.__len__()))
...
def main():
    ...
    parser.add_option(
        ...
        help=
        "combine all the df from 2 to 12, Total df 24. Using with -x AG,  Report size could be 6GB "
    )
