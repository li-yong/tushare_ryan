...
def main():
    ...
    parser.add_option("-f", "--fetch_index", action="store_true",
                      dest="fetch_index_f", default=False,
                      help="fetch SH000001")

    parser.add_option( "-v", "--verify_weekly_buy_sel_strategy", action="store_true",
                      dest="verify_weekly_buy_sel_strategy_f", default=False,
    ...
    if options.verify_weekly_buy_sel_strategy_f:
        ...
        df = pd.read_csv(csv_file, skiprows=1, header=None, names=['code', 'date', 'close', 'open', 'high', 'low',
