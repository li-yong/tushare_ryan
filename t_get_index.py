...
df = pd.read_csv(csv_file, converters={'code': str})
for index, row in df.iterrows():
    _d, o, c = row['trade_date'], row['close'], row['open']
    # print(_d, o, c)
    d = datetime.datetime.strptime(str(_d), '%Y%m%d')
    y = d.year
    ...
    if d.weekday() == 0:  # Monday is 0 and Sunday is 6.
        print("Buy at Monday close. " + str(_d) + ", " + str(c) + ", cap " + str(cap))
    ...
    if stock_num > 0 and d.weekday() == 3:  # sell on Thursday open
        cap = stock_num * o
        stock_num = 0
        print("sell at Friday close, " + str(_d) + ", " + str(c) + ", cap " + str(cap))
        x_axis.append(d)
        y_axis.append(cap)
...
print(" Index SH000001 saved to " + csv_file)
print("Done!")
os._exit(0)
