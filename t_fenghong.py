...
the_latest_report_date = finlib.Finlib().get_year_month_quarter(
)['last_quarter']
...
def get_reference_data():
    ...
    for year in range(
            2017, year_q['year'] + 1
    ):  #should not (2010,2019) as it will overwrite the 1980 to 2010 data.
        #for year in range(2018,2019): #debug
        ######################## ts.profit_data  ###################
        sys.stdout.write('profit_data year ' + str(year) + ". ")
        sys.stdout.flush()

        dump_reference_profit_data = "/home/ryan/DATA/pickle/Stock_Fundamental/fenghong/profit_data/pd_" + str(
            year) + ".csv"
        try:
            ...
            if (year > 2008) and (
                (not os.path.isfile(dump_reference_profit_data)) or
                (year == year_q['last_quarter']['year']
                 and quarter == year_q['last_quarter']['quarter'])):
                #if (year > 2008) and ((not os.path.isfile(dump_reference_profit_data))):
                #logging.info("\tGetting profit_data, ts.profit_data\t")  # 分配预案
                sys.stdout.write("getting profit_data, ts.profit_data. year " +
                                 str(year))
                sys.stdout.flush()
                df_tmp = ts.profit_data(year=year,
                                        top=30000,
                                        retry_count=10,
                                        pause=0.001)
                df_tmp.to_csv(dump_reference_profit_data,
                              encoding='UTF-8',
                              index=False)
                logging.info(__file__ + ": " + "saved, " +
                             dump_reference_profit_data)
                ...
                df_tmp = pandas.read_csv(dump_reference_profit_data,
                                         converters={'code': str})

        except:
            logging.info("\tcaught exception, profit_data, year " + str(year) +
                         ", " + dump_reference_profit_data)
            exit(0)
        finally:
            ...
            logging.info(__file__ + ": " + "df_profit_all saved, " +
                         profit_csv + " . len " + str(df_profit_all.__len__()))
        ...
        for quarter in range(1, 5):
            ...
            #sys.stdout.flush()
            dump_reference_forecast_data = "/home/ryan/DATA/pickle/Stock_Fundamental/fenghong/forecast/fd_" + str(
                year) + "_" + str(quarter) + ".csv"
            try:
                exc_info = sys.exc_info()
                df_tmp = pd.DataFrame()

                if (year > 2010) and (
                    (not os.path.isfile(dump_reference_forecast_data)) or
                    (year == year_q['last_quarter']['year']
                     and quarter == year_q['last_quarter']['quarter'])):
                    #if (year > 2010) and ((not os.path.isfile(dump_reference_forecast_data))):
                    sys.stdout.write(
                        "getting forecast_data, ts.forecast_data, year " +
                        str(year) + " quarter " + str(quarter))  # 业绩预告
                    sys.stdout.flush()
                    df_tmp = ts.forecast_data(year, quarter)
                    df_tmp.to_csv(dump_reference_forecast_data,
                                  encoding='UTF-8',
                                  index=False)
                    logging.info(__file__ + ": " + "saved, " +
                                 dump_reference_forecast_data)
                    ...
                    df_tmp = pandas.read_csv(dump_reference_forecast_data,
                                             converters={'code': str})

            except:
                logging.info("\tcaught exception, forecast_data, year " +
                             str(year) + " quarter " + str(quarter))
            finally:
                df_forecast_all = df_forecast_all.append(df_tmp,
                                                         ignore_index=True)
                ...
                df_forecast_all.to_csv(forecast_csv,
                                       encoding='UTF-8',
                                       index=False)
                logging.info(__file__ + ": " + "saved, " + forecast_csv +
                             " . len " + str(df_forecast_all.__len__()))
        ...
        for quarter in range(1, 5):
            ...
            dump_reference_fund_holdings = "/home/ryan/DATA/pickle/Stock_Fundamental/fenghong/fund_hold/fh_" + str(
                year) + "_" + str(quarter) + ".csv"
            try:
                exc_info = sys.exc_info()
                df_tmp = pd.DataFrame()
                ...
                if (year > 2010) and (
                    (not os.path.isfile(dump_reference_fund_holdings)) or
                    (year == year_q['last_quarter']['year']
                     and quarter == year_q['last_quarter']['quarter'])):
                    #if (year > 2010) and ((not os.path.isfile(dump_reference_fund_holdings))):
                    sys.stdout.write(
                        "\t\tgetting fund_holdings, ts.fund_holdings, year " +
                        str(year) + " quarter " + str(quarter))  # 基金持股
                    sys.stdout.flush()
                    df_tmp = ts.fund_holdings(year, quarter)
                    df_tmp.to_csv(dump_reference_fund_holdings,
                                  encoding='UTF-8',
                                  index=False)
                    logging.info(__file__ + ": " + "saved, " +
                                 dump_reference_fund_holdings)
                    ...
                    df_tmp = pandas.read_csv(dump_reference_fund_holdings,
                                             converters={'code': str})

            except:
                logging.info("\tcaught exception, fund_holdings, year " +
                             str(year) + " quarter " + str(quarter))
            finally:
                df_fundholding_all = df_fundholding_all.append(
                ...
                df_fundholding_all.to_csv(fundhold_csv,
                                          encoding='UTF-8',
                                          index=False)
                logging.info(__file__ + ": " + "saved, " + fundhold_csv +
                             " . len " + str(df_fundholding_all.__len__()))
        ...
        for month in range(1, 13):
            ...
            sys.stdout.flush()
            dump_reference_xsg_data = "/home/ryan/DATA/pickle/Stock_Fundamental/fenghong/restricted_lifted/rl_" + str(
                year) + "_" + str(month) + ".csv"
            ...
            try:
                ...
                if (year > 2010) and (
                    (not os.path.isfile(dump_reference_xsg_data))):
                    logging.info("\t\tgetting xsg_data, ts.xsg_data, year " +
                                 str(year) + " month " + str(month))  # 限售股解禁
                    df_tmp = ts.xsg_data(year, month)
                    df_tmp.to_csv(dump_reference_xsg_data,
                                  encoding='UTF-8',
                                  index=False)
                    logging.info(__file__ + ": " + "saved, " +
                                 dump_reference_xsg_data)

                    #df_tmp.to_pickle(dump_reference_xsg_data)
                elif os.path.isfile(dump_reference_xsg_data):
                    #sys.stdout.write("\tloading xsg_data")
                    df_tmp = pandas.read_csv(dump_reference_xsg_data,
                                             converters={'code': str})

            except:
                logging.info("\tcaught exception, xsg_data, year " +
                ...
                logging.info(__file__ + ": " + "saved, " + xsg_csv +
                             " . len " + str(df_xsg_all.__len__()))
...
def fetch_data_no_adj(debug=False, force_fetch=False):
    ...
    for code in list(df_code_name_map['code']):
        ...
        if (not os.path.isfile(csv_f)) or (os.stat(csv_f).st_size <=
                                           200):  # 200 bytes
            sys.stdout.write("file not exist or empty file, full fetch " +
                             csv_f)
            sys.stdout.flush()
            start_date_req = default_start_date
        else:
            ############ delta update start
            default_date_d = datetime.datetime.strptime(
                default_start_date, '%Y-%m-%d')
        ...
        sys.stdout.write("fetch " + csv_f + " " + str(i) + " of " +
                         str(df_code_name_map.__len__()))
        sys.stdout.flush()
        ...
        try:
            ...
            if df_hist_data is None:
                ...
                sys.stdout.write(" len fetched " +
                                 str(df_hist_data.__len__()) + " ")
        ...
        df_hist_data = df_hist_data.reindex(
            index=df_hist_data.index[::-1])  # revert
        ...
        cols = [
            'code', 'datetime', 'open', 'high', 'low', 'close', 'vol', 'amount'
        ]  # adjust column order
        df_hist_data = df_hist_data[cols]
        df_hist_data.columns = [
            'code', 'date', 'o', 'h', 'l', 'c', 'vol', 'amnt'
        ]  # rename column
        # convert date column from datetime to string
        df_hist_data['date'] = df_hist_data['date'].dt.strftime('%Y-%m-%d')

        df_hist_data = df_tmp.append(
            df_hist_data, ignore_index=True)  #append delta to existing data.

        df_hist_data.to_csv(csv_f, encoding='UTF-8', index=False)
        logging.info(__file__ + ": " + ". saved, len " +
                     str(df_hist_data.__len__()) + " " + csv_f)
...
def analyze_one(debug=False):
    ...
    df_profit = pd.DataFrame()
    if debug:
        df_profit = pd.read_csv(profit_csv_debug, converters={'code':
                                                              str})  #debug
    else:
        df_profit = pd.read_csv(profit_csv, converters={'code': str})
    ...
    df_result = pd.DataFrame(columns=[
        'code', 'name', 'year', 'report_date', 'divi', 'shares', 'price',
        'divi_percent', 'shares_percent'
    ])
    ...
    for i in range(df.__len__()):
        ...
        name = df.iloc[i, df.columns.get_loc('name')]  #
        year = df.iloc[i, df.columns.get_loc('year')]  #2003.0
        report_date = df.iloc[i,
                              df.columns.get_loc('report_date')]  #'2014-02-09'
        divi = df.iloc[i, df.columns.get_loc('divi')]  #1.5
        shares = df.iloc[i, df.columns.get_loc('shares')]  #0.0
        ...
        if not re.match("\d{4}", str(report_date)):
            continue
                ...
                "\d{4}",
                str(report_date)).group(0)) < 2014:  #pass year less than 2000
            continue

        report_dateSs = datetime.datetime.strptime(
            report_date, '%Y-%m-%d').strftime('%Y%m%d')
        last_trading_dateSs = finlib.Finlib().get_last_trading_day(
            report_dateSs)
        last_trading_date = datetime.datetime.strptime(
            last_trading_dateSs, '%Y%m%d').strftime('%Y-%m-%d')
        ...
        for i in range(-1, -10, -1):
            if price.__len__(
            ) == 0:  #in case the last_trading_date has no stock data
                slice_start_index = i
                slice_end_index = i + 1
                ...
                price = df_csv[df_csv['date'] <= last_trading_date][
                    slice_start_index:slice_end_index]['c']
        ...
        if price.__len__() == 0:
            logging.info(
                "WARNING:CANNOT GET HISTORY PRICE,  SHOULD NOT BE HERE. " +
                code + " " + last_trading_date)
            continue

        price = price.values[0]

        if price == 0:
            logging.info("Not continue as price is 0. " + code + " " +
                         last_trading_date)
            continue
        ...
        logging.info("code " + str(code) + " report date " + str(report_date) +
                     ".  div percent " + str(divi_percent) +
                     ", share percent " + str(share_percent))
...
def analyze_two():
    ...
    if (not force_run_global) and finlib.Finlib().is_cached(fenghong_score_csv,
                                                            day=5):
        logging.info("skip file, it been updated in 5 day. " +
                     fenghong_score_csv)
        return
    ...
    df_result = pd.DataFrame(columns=[
        'code', 'name', 'score', 'divi_m_score', 'share_m_score', 'fhcnt',
        'fhcnt_p', 'divi_sum', 'divi_p', 'share_sum', 'share_p', 'divi_m',
        'share_m'
    ])
    ...
    for i in range(leng):
        code = codes[i]
        logging.info('analyze_two, stage 1, ' + str(i) + " of " + str(leng) +
                     "," + str(code))
    ...
    for i in range(leng):
        ...
        logging.info('analyze_two, stage 2, ' + str(i) + " of " + str(leng) +
                     "," + str(code))
        ...
        df_result.iloc[i, df_result.columns.get_loc('divi_m_score'
                                                    )] = divi_m_perc
        df_result.iloc[i, df_result.columns.get_loc('share_m_score'
                                                    )] = share_m_perc
    ...
    cols = [
        'code', 'name', 'score', 'divi_m_score', 'share_m_score', 'fhcnt',
        'fhcnt_p', 'divi_sum', 'divi_p', 'share_sum', 'share_p'
    ]  # adjust column order, sort column order.
    df_result = df_result[cols]

    df_result = df_result.sort_values('score', ascending=False)
    df_result.to_csv(fenghong_score_csv, encoding='UTF-8', index=False)
    logging.info(__file__ + ": " + 'fenghong score result saved to ' +
                 fenghong_score_csv)
