...
df_daily_basic = finlib.Finlib().regular_read_csv_to_stdard_df(data_csv=fund2_dir+'/source/basic_daily/basic_'+date_exam_day+".csv")
df_daily_basic['volume_ratio_perc_rank'] = df_daily_basic['volume_ratio'].rank(pct=True) #量比
df_daily_basic['total_mv_perc_rank'] = df_daily_basic['total_mv'].rank(pct=True) #总市值 （万元）
df_daily_basic['circ_mv_perc_rank'] = df_daily_basic['circ_mv'].rank(pct=True) #流通市值（万元）
df_daily_basic['pe_perc_rank'] = df_daily_basic['pe'].rank(pct=True) #市盈率（总市值/净利润， 亏损的PE为空）
df_daily_basic['pe_ttm_perc_rank'] = df_daily_basic['pe_ttm'].rank(pct=True) #市盈率（TTM，亏损的PE为空）
df_daily_basic['ps_ttm_perc_rank'] = df_daily_basic['ps_ttm'].rank(pct=True) #市销率（TTM）
df_daily_basic['turnover_rate_f_perc_rank'] = df_daily_basic['turnover_rate_f'].rank(pct=True) #换手率（自由流通股）
#df_daily_basic['dv_ttm_perc_rank'] = df_daily_basic['dv_ttm'].rank(pct=True) #股息率（TTM）（%）
...
df_daily_price = finlib.Finlib().regular_read_csv_to_stdard_df(data_csv='/home/ryan/DATA/pickle/daily_update_source/ag_daily_'+date_exam_day+".csv")
...
df_exam_day_all = pd.merge(df_daily_basic, df_daily_price, on=['code','trade_date'], how='inner', suffixes=('', '_x'))
