...
def calc_daily_incease(input_csv, bv_csv):
    ...
    df = pd.read_csv(input_csv, skiprows=1, converters={'code': str}, header=None,
...
def verify(bv_csv, op, start_date=None, end_date=None, top_N = 5, days_after_signal_to_buy=1, buy_and_hold_days=10):
    ...
    if os.path.isfile(bv_csv):
        df = pd.read_csv(bv_csv, skiprows=1, converters={'c': float, 'date':str}, header=None,
                         names=['increase', 'code', 'date', 'o', 'h', 'l', 'c', 'vol', 'amt', 'tnv'])
    else:
    ...
    if op == "MISS_MAX_INCREASE":
        ...
        for i in range(df_top_n_increase.__len__()):
            ...
            if stock_count>0 and index_in_df + 1  < df.__len__() : #not the latest row.
                ...
                sys.stdout.write("sell at " + str(the_df_aft['date']) + " " + str(the_df_aft['c']) + ". profit " + str(
                    profit) + ".\n")
        ...
        for i in range(df_top_n_decrease.__len__()):
            ...
            sys.stdout.write(the_df['code'] + " " + the_df['date'] + " " + str(
                round(the_df['c'], 2)) + " IndayIncrease: " + str(round(the_df['increase'], 2)) + ".  ")
            ...
            if stock_count>0 and index_in_df + 1 < df.__len__():  # not the latest row.
                ...
                sys.stdout.write(
                    "sell at " + str(the_df_aft['date']) + " " + str(the_df_aft['c']) + ". profit " + str(
                        profit) + ".\n")
...
def main():
    ...
    parser.add_option("-t", "--top_N",  dest="top_n", default=5,type="int",
    ...
    parser.add_option("-s", "--start_date",  dest="start_date",type="str",
                      help="Optional. Verify start date. In format YYYY-MM-DD")

    parser.add_option("-e", "--end_date",  dest="end_date",type="str",
                      help="Optional. Verify end date")

    parser.add_option("-p", "--days_after_signal_to_buy", dest="days_after_signal_to_buy",type="int", default=1,
                      help="Optional. Using with CATCH operation.")


    parser.add_option("-q", "--buy_and_hold_days", dest="buy_and_hold_days",type="int", default=10,
                      help="Optional. Using with CATCH operation.")


    parser.add_option("-o", "--operation",  dest="operation",type="str",
                      help="MISS_MAX_INCREASE|MISS_MAX_DECREASE|MISS_MAX_INDECREASE|CATCH_MAX_INCREASE|CATCH_MAX_DECREASE")
    ...
    for f in files_path:
        ...
        (code, start_date,end_date, profit, bhf_profit) = verify(bv_csv=bv_csv, op=operation_f, start_date=start_date_f, end_date=end_date_f,
               days_after_signal_to_buy=days_after_signal_to_buy_f, buy_and_hold_days=buy_and_hold_days_f,
               top_N=top_n_f)
