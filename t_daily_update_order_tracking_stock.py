...
for i in range(open_stocks.__len__()):
    ...
    if int(buy_price) == 0:
        print("not update record as buy_price is 0, record " + str(id) + ", " +
              code)
        continue
    ...
    if os.path.isfile(csv_f):
        ...
        for i in (1, 2, 3, 5, 10, 15, 20, 60, 120, 240):
            ...
            if date_i_row_index < df_csv.__len__():
                print("\t" + code + " have data on Buy_date(" + str(buy_date) +
                      ") + " + str(i) + ", " + str(eval("date_" + str(i))))
                date_i_row = df_csv.iloc[buy_date_index + i]
                day_price = date_i_row['c']
                if int(day_price) == 0:  #ting pai
                    day_price = buy_price

                data_sql['D' + str(i) + '_inc'] = round(
                    (day_price - buy_price) / buy_price * 100, 1)

                #SZZS increase% at the same period
                if szzs_buy_date_index + i >= df_index.__len__():
                    print(
                        "ERROR: the index doesn't have the date record, run t_get_index.py"
                    )
                    pass
                else:
                    szzs_date_i_row = df_index.iloc[szzs_buy_date_index + i]
                    data_sql['ID_' + str(i)] = round(
                        (szzs_date_i_row['c'] - szzs_buy_date_close) /
                        szzs_buy_date_close * 100, 1)
                    pass
...
D1_inc_avg = round(sql_rtn.iloc[0, sql_rtn.columns.get_loc('avg(`D1_inc`)')],
                   2)
D2_inc_avg = round(sql_rtn.iloc[0, sql_rtn.columns.get_loc('avg(`D2_inc`)')],
                   2)
D3_inc_avg = round(sql_rtn.iloc[0, sql_rtn.columns.get_loc('avg(`D3_inc`)')],
                   2)
D5_inc_avg = round(sql_rtn.iloc[0, sql_rtn.columns.get_loc('avg(`D5_inc`)')],
                   2)
D10_inc_avg = round(
    sql_rtn.iloc[0, sql_rtn.columns.get_loc('avg(`D10_inc`)')], 2)
D15_inc_avg = round(
    sql_rtn.iloc[0, sql_rtn.columns.get_loc('avg(`D15_inc`)')], 2)
D20_inc_avg = round(
    sql_rtn.iloc[0, sql_rtn.columns.get_loc('avg(`D20_inc`)')], 2)
D60_inc_avg = round(
    sql_rtn.iloc[0, sql_rtn.columns.get_loc('avg(`D60_inc`)')], 2)
D120_inc_avg = round(
    sql_rtn.iloc[0, sql_rtn.columns.get_loc('avg(`D120_inc`)')], 2)
D240_inc_avg = round(
    sql_rtn.iloc[0, sql_rtn.columns.get_loc('avg(`D240_inc`)')], 2)
...
ID10_inc_avg = round(
    sql_rtn.iloc[0, sql_rtn.columns.get_loc('avg(`ID_10`)')], 2)
ID15_inc_avg = round(
    sql_rtn.iloc[0, sql_rtn.columns.get_loc('avg(`ID_15`)')], 2)
ID20_inc_avg = round(
    sql_rtn.iloc[0, sql_rtn.columns.get_loc('avg(`ID_20`)')], 2)
ID60_inc_avg = round(
    sql_rtn.iloc[0, sql_rtn.columns.get_loc('avg(`ID_60`)')], 2)
ID120_inc_avg = round(
    sql_rtn.iloc[0, sql_rtn.columns.get_loc('avg(`ID_120`)')], 2)
ID240_inc_avg = round(
    sql_rtn.iloc[0, sql_rtn.columns.get_loc('avg(`ID_240`)')], 2)

print('D1_increase_avg: ' + str(D1_inc_avg) + "%" + ", index inc avg: " +
      str(ID1_inc_avg) + "%")
print('D2_increase_avg: ' + str(D2_inc_avg) + "%" + ", index inc avg: " +
      str(ID2_inc_avg) + "%")
print('D3_increase_avg: ' + str(D3_inc_avg) + "%" + ", index inc avg: " +
      str(ID3_inc_avg) + "%")
print('D5_increase_avg: ' + str(D5_inc_avg) + "%" + ", index inc avg: " +
      str(ID5_inc_avg) + "%")
print('D10_increase_avg: ' + str(D10_inc_avg) + "%" + ", index inc avg: " +
      str(ID10_inc_avg) + "%")
print('D20_increase_avg: ' + str(D20_inc_avg) + "%" + ", index inc avg: " +
      str(ID20_inc_avg) + "%")
print('D60_increase_avg: ' + str(D60_inc_avg) + "%" + ", index inc avg: " +
      str(ID60_inc_avg) + "%")
print('D120_increase_avg: ' + str(D120_inc_avg) + "%" + ", index inc avg: " +
      str(ID120_inc_avg) + "%")
print('D240_increase_avg: ' + str(D240_inc_avg) + "%" + ", index inc avg: " +
      str(ID240_inc_avg) + "%")
