...
class CursedUI(object):
    ...
    def Update(self, datapoint):
        ...
        if self._isHeartbeatUpdate(datapoint):
            ...
            string = str(datapoint["time"])+",";
            string += str(self._currentPrice);

            #logging.info("ticker: "+string)
            filename = self._instrument + ".csv"
            logging.info("open ticker file "+filename)
            self.tickerfile=open(filename, 'a')
            self.tickerfile.write(string+"\n")
    ...
    def ProcessUserInput(self):
        ...
        if c == ord('b'):
            ...
            self._strategy.Buy(self._heartbeatTime)
        ...
        if c == ord('s'):
            ...
            tstatus = self._strategy.TradingStatus()
            self._strategy.ResumeTrading()
            self._strategy.Sell(self._heartbeatTime)
            self._strategy.SetTradingStatus(tstatus)
    ...
    def Render(self):
       ...
       # self.stdscr.addstr(25,0,"Ticker:    "+str(self._currentPrice))
