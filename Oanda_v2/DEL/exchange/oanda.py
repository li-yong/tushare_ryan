...
class Oanda(object):
    def __init__(self, api_key, account_id, instrument, account_currency, \
                 home_base_pair, home_base_default_exchange_rate = 1.0, environment="practice", \
        ...
        request_params = {}
        api_v20 = API(access_token=token,
                  environment="practice",
                  request_params=request_params)

        self._api_v20 = api_v20
    ...
    def ClosePosition(self):
        trades = self._oanda.get_trades(self._account_id)
        for t in trades['trades']:
            tid = t['id']
            logging.info("closing trading id: " + str(tid))
            ct = self._oanda.close_trade(self._account_id, tid)
    ...
    def GetCandles(self, number_of_last_candles_to_get = 0, size_of_candles_in_minutes = 120):
        ...
        for item in response["candles"]:
            ...
            # ryan add start, 2018.Feb, 04.
            c.Volume = item["volume"]
            c.Complete = item["complete"]
            c.Time = item["time"]
            c.Instrument = response['instrument']
            c.Granularity = response['granularity']
            # ryan add end, 2018.Feb, 04.
    ...
    def _getGranularity(self, size_in_minutes):
        if size_in_minutes == 5.0/60: #ryan add start 2018.02.04
            return "S5"
        if size_in_minutes == 10.0/60:
            return "S10"
        if size_in_minutes == 15.0/60:
            return "S15"
        if size_in_minutes == 30.0/60:
            return "S30"
        if size_in_minutes == 1: #ryan add end 2018.02.04
            return "M1"
        ...
        if size_in_minutes == 2:
            ...
            return "H8"
        elif size_in_minutes == 60*12: #ryan add 2018.02.04
            return "H12"
        elif size_in_minutes == 1440:
            return "D1"
        elif size_in_minutes == 60*24*7:  #ryan add 2018.02.04
            return "W"
        elif size_in_minutes == 60*24*7*30:  #ryan add 2018.02.04
            return "M"
